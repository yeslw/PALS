...
LOCAL_MODEL_PATH = "/2T/zhuhe/data/decapoda-research-llama-7B-hf"
...
def main():
    ...
    parser.add_argument('--model', type=str, default=LOCAL_MODEL_PATH, help='LLaMA model')
    ...
    if "opt" in args.model:
        tokenizer = GPT2Tokenizer.from_pretrained(args.model, use_fast=False, local_files_only=True)
    else:
    ...
    if "dlp" in args.prune_method:
        ...
        print("\n[DLP] Layer-wise Sparsity Ratios:")
        print(f"{'Layer':<10} {'Sparsity':<15} {'Density (imp_ratio)':<20}")
        print("-" * 45)
        for i, ratio in enumerate(imp_ratio):
            sparsity = 1.0 - ratio
            print(f"{i:<10} {sparsity:.4f}           {ratio:.4f}")
        print("-" * 45)
  ...
  ############################ owl   ############################
    ...
    print("*"*30)
    ################################################################
    ppl_test = eval_ppl(model, tokenizer, device, dataset="wikitext2")
    print(f"ppl on wikitext2 {ppl_test}")
